...
/**
 * This plugin collects helpers for all resources.
 *
 * Every helper file becomes a key in the decorated `helpers` object and
 ...
 * @param {string} opts.dirName - directory to load helper defining plugins from
 ...
 * @async
 */
async function loadHelpersAsync(fastify, opts) {
  ...
  if (dirName) {
    fastify.register(importModules, {
      ...
      decoratorName: 'helpers'
    });
  }

  await fastify.after();
  const loadedHelpers = Object.keys(fastify.helpers).join(', ');
  fastify.log.info(`Loaded helpers: ${loadedHelpers}`);
}

export default fp(loadHelpersAsync, {
  name: moduleName
});
